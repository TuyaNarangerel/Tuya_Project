...
namespace Tuya_Project1.Menu
{
    public class ShapeMenu
    {
		...
		private void UpdateShape()
		{
			...
			if (IsValidInput(newBaseInput))
			{
				...
				if (IsValidInput(newHeightInput))
				{
					...
					if (shape.Type == "Rectangle" && newBase == newHeight)
					{
						Console.WriteLine("The base and height of a rectangle cannot be the same.");
					}
					else if (shape.Type == "Parallelogram" && newHeight <= newBase)
					{
						Console.WriteLine("The height of a parallelogram must be greater than its base.");
					}
					else
					{
						shape.Base = newBase;
		...
		private void AddRectangle()
		{
			...
			if (IsValidInput(widthInput))
			{
				...
				if (IsValidInput(heightInput))
				{
					...
					{
						var existingRectangle = _context.Shapes.FirstOrDefault(s => s.Type == "Rectangle" && s.Base == width && s.Height == height);

						if (existingRectangle != null)
						{
							Console.WriteLine("A similar rectangle already exists in the database.");
		...
		{
			Console.Write("Enter parallelogram base: ");
			...
			if (IsValidInput(baseInput))
			{
				...
				if (IsValidInput(heightInput))
				{
					...
					else
					{
						...
						{
							Console.WriteLine("A similar parallelogram already exists in the database.");
						}
						else
						{
